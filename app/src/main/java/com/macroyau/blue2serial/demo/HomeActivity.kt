...
class HomeActivity : BluetoothSerialActivityBase() {
	inner class Timing(
		...
		fun remove() {
			val ll = findViewById<View>(R.id.list_times) as LinearLayout
			ll.removeView(view)
		}
	...
	companion object {
		...
		private const val TAG = "StofzuigerRobotHome"
	}
	...
	@Suppress("MemberVisibilityCanBePrivate")
	var times: MutableList<Timing>? = null
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		totalTimes = 0
		times = ArrayList()
		fan_on!!.setOnCheckedChangeListener { _, b ->
			setCfg(CFG_FAN_ON, if (b) 1 else 0)
		}
	...
	override fun onOptionsItemSelected(item: MenuItem): Boolean {
		val id = item.itemId
		if (id == R.id.terminal) {
			val intent = Intent(this@HomeActivity, TerminalActivity::class.java)
	...
	@Suppress("MemberVisibilityCanBePrivate")
	fun setCfg(addr: Int, `val`: Int) {
		setCfg(addr, `val`.toString())
	}

	@Suppress("MemberVisibilityCanBePrivate")
	fun setCfg(addr: Int, `val`: String) {
		exec("c$addr;$`val`;")
	}
